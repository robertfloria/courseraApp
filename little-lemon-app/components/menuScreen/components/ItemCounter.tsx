...
export default function ItemCounter({ counter, setCounter }: Props) {
    ...
    return (
        <View style={styles.container}>
            <Pressable style={[{ backgroundColor: backgroundColor }, styles.button]} onPress={decrease} disabled={isButtonDisabled}>
                <MaterialIcons name='exposure-minus-1' size={20} color={color} />
            </Pressable>
            <ThemedText style={{ fontSize: 60, lineHeight: 0 }}>{counter}</ThemedText>
            <Pressable style={[{ backgroundColor: backgroundColor }, styles.button]} onPress={increase}>
